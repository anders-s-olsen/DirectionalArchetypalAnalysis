...
%     Xs = X./sqrt(sum(X.^2,2)); %Tjek

    % Determine dominant direction (dd)
    [coeff, dd] = pca(X, 'NumComponents',1); %X til Xs
    to_flip = 2*(coeff'<0)-1;
    Xf = bsxfun(@times, X, to_flip); % X til Xs
    dd = normc(dd);
    XC=Xf(:, I)*C; 
    ...
    figure('Position',[50,250,1400,650]),
    while abs(dloss)>=conv_crit*abs(sum(loss)) && iter<maxiter
        ...
        loss_old = loss;
        % C update
        [C,loss,muC,XC,g]=Cupdate(X,Xf,C,S,muC,loss,niter(1),loss_fn,I); 
        % S update            
        ...
        if rem(iter,10)==0
            ...
            figure(1),clf,
            subplot(2,3,1),plot(g),
            subplot(2,3,4),plot(C),
            subplot(2,3,2),plot(XC),
            subplot(2,3,3),plot(S'),
            shg
        end
    end
...
function [C,loss,muC,XC,g]=Cupdate(X,Xf,C,S,muC,loss,niter,loss_fn,I)
    for k=1:niter
        loss_old=loss;
...
% 
%         g = -2*Xf(:,I)'*TV*S';
%         g = bsxfun(@minus,g,sum(bsxfun(@times,g,C)*sign(C)));
%         stop=0;
%         C_old=C;
%         while ~stop
%             C = C_old-bsxfun(@times,g,muC);
%             
%             C=bsxfun(@rdivide,C,sum(abs(C))+eps);
% 
%             XC=Xf(:, I)*C;
%             XCS=XC*S;           
%             q   = sum(XCS.^2);
